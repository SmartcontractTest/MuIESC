...
spender.receiveApproval(msg.sender,uint128(_value), this, _extraData);//Mutation Here for <74>
            return true;
        }
    }
...
contract FujintoToken is owned, token {
    ...
    function buy() payable {
		...
		if (msg.value == 0 || buyRate == 0 || amount/buyRate != msg.value) revert();  //<repair>
		if (balanceOf[msg.sender] + amount < amount) revert(); //<repair>
        balanceOf[msg.sender] += amount;  // <overflow>                // adds the amount to buyer's balance
		if (balanceOf[owner] < amount) revert(); //<repair>
