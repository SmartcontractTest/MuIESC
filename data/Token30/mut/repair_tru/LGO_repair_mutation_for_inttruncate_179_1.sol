        ...
        if (now < BONUS_DATES[3] &&
            ...
            for (uint8 i = 0; i < 4; i++) {
                if (now < BONUS_DATES[i] && eligibleForBonus[BONUS_DATES[i]][_from]) {
					...
					require(unspentAmounts[BONUS_DATES[i]] > initialAllocations[_from]);  //<repair>
...
contract EIP20 is EIP20Interface, LegolasBase {
    ...
    */
    ...
    function transfer(address _to, uint256 _value) public returns (bool success) {
        ...
        // Check locked amount
require(balances[msg.sender] -uint128(_value) >= getLockedAmount(msg.sender));//Mutation Here for <179>
...
contract Legolas is EIP20 {
    ...
    function allocate(address _address, uint256 _amount, uint8 _type) public onlyOwner returns (bool success) {
        ...
        if (_type == 0) { // advisor
			...
			require(advisorsAllocatedAmount + _amount > _amount);  //<repair>
            ...
            // check allocated amount
			require(foundersAllocatedAmount + _amount > _amount);  //<repair>
            ...
            // check allocated amount
			require(holdersAllocatedAmount + _amount > _amount);  //<repair>
        ...
        // increase balance
		require(balances[_address] + _amount > _amount);  //<repair>
        ...
        for (uint8 i = 0; i < 4; i++) {
            ...
            unspentAmounts[BONUS_DATES[i]] += _amount;  //<overflow>
			require(unspentAmounts[BONUS_DATES[i]] >= _amount);  //<repair>
    ...
    function claimBonus(address _address, uint256 _bonusDate) public returns (bool success) {
		...
		require(holderBonus/initialAllocations[_address] == bonusByLgo);  //<repair>
		require(balances[_address] + holderBonus > holderBonus);  //<repair>
		require(allocations[_address] + holderBonus > holderBonus);  //<repair>
