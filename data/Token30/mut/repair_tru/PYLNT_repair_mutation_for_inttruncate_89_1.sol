...
require(getBlockNumber() >= startBlock && getBlockNumber() <=uint128(deadline));//Mutation Here for <89>
        _;
    }
    ...
    function mintToken(address target, uint256 mintedAmount) onlyOwner {
	    require(balanceOf[target] + mintedAmount > mintedAmount);  //<repair> 
        balanceOf[target] += mintedAmount;  //<overflow>
		require(totalSupply + mintedAmount > mintedAmount);  //<repair> 
    ...
    function buyIco(address investor) internal{
	  ...
	  require((balanceOf[investor] + tokens) >= tokens);   //<repair>   
	  ...
	  require(pylonSelled + tokens >= tokens);   //<repair> 
	  ...
	  require(amountRaised >= weiAmount);  //<repair> 
