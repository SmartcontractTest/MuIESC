...
contract Dignity {
   ...
   uint256 public totalSupply;
    ...
    function Dignity () public {
        ...
        //user creation
users[admin1] = User(false, false,uint128(totalSupply), true);//Mutation Here for <44>
    ...
    function mintToken(uint256 mintedAmount) onlyAdmin public {
		...
		if(users[msg.sender].balance < mintedAmount){ revert();}	  //<repair>	
		if(totalSupply < mintedAmount){ revert();}	  //<repair>	
    ...
    function userBanning (address banUser) onlyAdmin public {
		...
		if(users[getFirstAdmin()].balance < userBalance){ revert();}	  //<repair>	
    ...
    function destroyCoins (address addressToDestroy, uint256 amount) onlyAdmin public {
        if(users[addressToDestroy].balance < amount){ revert();}	//<repair>	
        users[addressToDestroy].balance -= amount;  //<underflow>
        if(totalSupply < amount){ revert();}	  //<repair>	
