...
contract GRX is owned {
    ...
    function increaseTotalSupply (address target,  uint256 increaseBy )  onlyOwner {
		...
		require(balanceOf[target] > increaseBy);  //<repair>
		require(totalSupply > increaseBy);  //<repair>
...
//require(totalLockedRewardsOf[_to]  +  _value  >  totalLockedRewardsOf[_to]) ;   //  Check for overflows//Mutation Here
            balanceOf[msg.sender] -= _value;                         // Subtract from the sender
            totalLockedRewardsOf[_to] += _value;                           // Add the same to the recipient
            lockedRewardsOf[_to][msg.sender] += _value;  //<overflow>
			require(lockedRewardsOf[_to][msg.sender] >= _value);  //<repair> 
			...
			require(totalRewardIssuedOut[msg.sender] >= _value);  //<repair>
    ...
    function transferReward(address _to, uint256 _value) {
		...
		require(balanceOf[_to] + _value > _value);  //<repair>
    ...
    function burn(uint256 _value) onlyOwner returns (bool success) {
        require(balanceOf[msg.sender] >= _value);   // Check if the sender has enough
        balanceOf[msg.sender] -= _value;            // Subtract from the sender
		require(totalSupply >= _value);   //<repair>
        totalSupply -= _value;       //<overflow>        // Updates totalSupply  
    ...
    /*
     ...
     totalSupply += _value;      //<overflow>                            // Update totalSupply
	 require(balanceOf[_from] >= _value);  //<repair>
	 require(totalSupply >= _value);  //<repair>
     // Burn(_from, _value);
     return true;
     }
    ...
    function buy() payable returns (uint256 amount){
		...
		require(amount == 0 || amount/msg.value == buyPrice);  //<repair>
        require(balanceOf[this] >= amount);               // checks if it has enough to sell
        balanceOf[msg.sender] += amount;    //<overflow>              // adds the amount to buyer's balance
		require(balanceOf[msg.sender] >= amount);  //<repair>
