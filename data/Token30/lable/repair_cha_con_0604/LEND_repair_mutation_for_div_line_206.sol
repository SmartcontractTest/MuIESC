...
/// Functions:
    ...
    function EthLendToken(address _tokenManager, address _escrow, address _teamTokenBonus) 
    {
        ...
        balances[_teamTokenBonus] += teamBonus;  //<overflow>
        supply+= teamBonus;  //<overflow>
		require( balances[_teamTokenBonus] >= teamBonus);  //<repair>
		require( supply >= teamBonus);  //<repair>

        assert(PRESALE_TOKEN_SUPPLY_LIMIT==60000000 * (1 ether / 1 wei));
assert(TOTAL_SOLD_TOKEN_SUPPLY_LIMIT == 1000000000  *  (1 ether  *  1 wei)) ;//Mutation Here for<None>
    }
    ...
    function buyTokensPresale() public payable onlyInState(State.PresaleRunning)
    {
        ...
        uint newTokens = msg.value * PRESALE_PRICE;  //<overflow>
		require(newTokens/PRESALE_PRICE == msg.value);  // <repair>
		require(presaleSoldTokens + newTokens > presaleSoldTokens );  // <repair>
        require(presaleSoldTokens + newTokens <= PRESALE_TOKEN_SUPPLY_LIMIT);  //<overflow>

		require(balances[msg.sender] + newTokens > newTokens);  // <repair>
        balances[msg.sender] += newTokens;  //<overflow>
		require(supply + newTokens > newTokens);  // <repair>
        supply+= newTokens;  //<overflow>
		require(presaleSoldTokens + newTokens > newTokens);  // <repair>
        presaleSoldTokens+= newTokens;  //<overflow>
		require(totalSoldTokens + newTokens > newTokens);  // <repair>
        totalSoldTokens+= newTokens;  //<overflow>
    ...
    function buyTokensICO() public payable onlyInState(State.ICORunning)
    {
        ...
        uint newTokens = msg.value * getPrice();  //<overflow>
		require(newTokens/msg.value == getPrice());  //<repair>
		require(totalSoldTokens + newTokens > newTokens);  //<repair>
        require(totalSoldTokens + newTokens <= TOTAL_SOLD_TOKEN_SUPPLY_LIMIT);  //<overflow>

        require(balances[msg.sender] + newTokens > newTokens);  // <repair>
		balances[msg.sender] += newTokens;  //<overflow>
		require(supply + newTokens > newTokens);  // <repair>
        supply+= newTokens;  //<overflow>
		require(icoSoldTokens + newTokens > newTokens);  // <repair>
        icoSoldTokens+= newTokens;  //<overflow>
		require(totalSoldTokens + newTokens > newTokens);  // <repair>
        totalSoldTokens+= newTokens;  //<overflow>
